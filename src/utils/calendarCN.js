...
const creatLenArr = (year, month, len, start) => {
  const arr = [];
  start = start || 0;
  if (len < 1) return arr;
  let k = start;
...
function getLunarYearDays(year) {
  ...
  const monthDataArr = monthData.split('').map((item) => parseInt(item, 10));
...
function getLunarByBetween(year, month, day) {
  ...
  const between = getDaysBetweenSolar(
    year,
    zenMonth - 1,
    zenDay,
    year,
...
function formatDate(year, month, day, _minYear) {
  ...
  if (year < (_minYear || minYear + 1) || year > maxYear)
...
function solarToLunar(_year, _month, _day) {
  ...
  // 立春日期
  const term2 = cacheUtil.get('term2')
    ? cacheUtil.get('term2')
    : cacheUtil.set('term2', getTerm(year, 2));
  // 二十四节气
  const termList = cacheUtil.get('termList')
  ...
  return {
    ...
    // 放假安排：0无特殊安排，1工作，2放假
    worktime:
      worktime[`y${year}`] && worktime[`y${year}`][formatDayD4(month, day)]
        ? worktime[`y${year}`][formatDayD4(month, day)]
        : 0,
